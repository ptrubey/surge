...
# inputs = read.csv('~/git/projgamma/datasets/slosh/slosh_params.csv')
...
theta_t90 = theta[which(as.logical(thresholded$t90)),][c('slr','theta','v','pmin','lat')]
# pt = ggpairs(
#   theta_t90,
...
#   columnLabels = c('Sea Level Rise', 'Approach Angle', 
#                   'Approach Speed', 'Minimum Pressure', 'Latitude')
# )
pt = ggpairs(
  theta_t90,
  columns = c('slr','theta','v','pmin','lat'),
  upper = list(continuous = 'blank'), 
  lower = list(continuous = wrap('density', adjust = 1.5, color = 'darkgrey')), 
  diag = list(continuous = wrap("barDiag", bins = 50)),
  columnLabels = c('Sea Level Rise', 'Approach Angle', 
                   'Approach Speed', 'Minimum Pressure', 'Latitude')
)
print(pt)
# ggsave(pt, file = '~/git/exapg/plots/threshold_pairs.pdf', 
#        device = cairo_pdf, width = 8, height = 8, units = 'in')
# ggsave(pt, file = '~/git/dissertation/ch3/plots/threshold_pairs.pdf', 
#        device = cairo_pdf, width = 8, height = 8, units = 'in')
...
theta$radians = (360 - theta$theta) * pi / 180 + pi/2
max_ran = max(theta$radians)
min_ran = min(theta$radians)
mid_ran = mean(c(max_ran, min_ran))
r = 0.2
upper1 = c(r * cos(mid_ran), r * sin(mid_ran)) + upper2 
# upper3 = c(r * cos(min_ran), r * sin(min_ran)) + upper2
lower1 = c(r * cos(mid_ran), r * sin(mid_ran)) + lower2
# lower3 = c(r * cos(min_ran), r * sin(min_ran)) + lower2
# bounds = data.frame(rbind(upper1,upper2,upper3,lower1,lower2,lower3))
bounds = data.frame(rbind(upper1, upper2, lower1, lower2))
names(bounds) = c('long','lat')
bounds$spec = c(1, 1, 2, 2)
arrow_vertex = theta[c('long','lat')][1,]
arrow_dir = (360 - theta$theta[1]) * pi / 180 + pi / 2
arrow_r = 0.5
arrow_v = 0.1
arrow_t = pi/2
arrow_bottom = c(- arrow_r * cos(arrow_dir), - arrow_r * sin(arrow_dir)) + arrow_vertex
arrow_l = c(arrow_v * cos(arrow_dir - (pi - arrow_t / 2)), arrow_v * sin(arrow_dir - (pi - arrow_t / 2))) + arrow_vertex
arrow_u = c(arrow_v * cos(arrow_dir + (pi - arrow_t / 2)), arrow_v * sin(arrow_dir + (pi - arrow_t / 2))) + arrow_vertex

arrow = data.frame(rbind(arrow_vertex, arrow_l, arrow_vertex, arrow_u, arrow_vertex, arrow_bottom))
names(arrow) = c('long','lat')
arrow$spec = c(1,1,2,2,3,3)
...
mainplt = ggplot(data = states, aes(x = long, y = lat)) +
  ...
  scale_fill_viridis_c(limits = c(0, max(slosh$slosh2)), breaks = c(0,2,4,6,8)) +
  bbox(slosh) +
  theme_minimal() + xlab(element_blank()) + ylab(element_blank()) +
  theme(legend.title = element_blank(), legend.key.height = unit(2.5,'cm')) +
  # theme(
  ...
  geom_line(
    data = bounds, 
    mapping = aes(x = long, y = lat, group = spec), 
    color = 'red', 
    linewidth = 2.
    ) +
  geom_line(
...
combined = ggarrange(
  mainplt, plt_t90, ncol = 2, nrow = 1, 
  common.legend = TRUE, legend = 'right'
  )
# ggsave('~/Desktop/temp.pdf', combined, height = 6, width = 10, units = 'in')
ggsave('~/git/exapg/plots/slosh_combined.pdf', combined, height = 6, width = 10, units = 'in')
ggsave('~/git/dissertation/ch3/plots/slosh_combined.pdf', combined, height = 6, width = 10, units = 'in')
